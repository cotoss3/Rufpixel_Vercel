...
import { ShoppingBag, Menu, X, Phone, Mail, Clock, Calculator, Search, ShieldCheck } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 shadow-2xl">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex justify-between items-center">
          <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-2">
              ...
              <span>+507 6000-0000 / +507 300-0000</span>
            </div>
            <div className="flex items-center space-x-2">
      ...
      <div className="bg-[#0D0D0D] border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-28 flex justify-between items-center">
          {/* Brand Logo */}
          <Link href="/" className="flex items-center group h-full py-0 my-0 -ml-1">
            <img
              ...
              return (
                <Link
                  ...
                  className={`px-3.5 py-2 rounded-lg text-sm font-medium transition-all ${
                    active
                      ? 'text-[#FF5E14] bg-white/5 font-semibold'
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link
              href="/cotizador"
              className="flex items-center space-x-1.5 bg-[#FF5E14] hover:bg-[#E04700] text-white px-2.5 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm font-bold shadow-md shadow-[#FF5E14]/25 transition-all"
            >
              <Calculator className="w-4 h-4 shrink-0" />
              <span className="hidden xs:inline sm:inline">Pre-orden</span>
              <span className="xs:hidden sm:hidden font-extrabold">Cotizar</span>
            </Link>
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-[#0D0D0D] border-b border-gray-800 px-4 pt-3 pb-6 space-y-2">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
          ...
          <div className="pt-4 border-t border-gray-800 flex flex-col space-y-3">
            <Link
              href="/cotizador"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full flex items-center justify-center space-x-2 bg-[#FF5E14] text-white py-3 rounded-lg font-bold"
            >
